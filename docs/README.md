# Index Documentation

## Documents de pilotage

- [`CURRENT.md`](CURRENT.md)
- [`sprint-planning.md`](sprint-planning.md)
- [`../README.md`](../README.md) — vue globale et etat des sprints (section « Où en est le projet »)
- [`sprints/sprint-0-ansible.md`](sprints/sprint-0-ansible.md)
- [`sprints/sprint-1-gitops-local.md`](sprints/sprint-1-gitops-local.md)
- [`CONTRIBUTING.md`](CONTRIBUTING.md)

## Documents techniques

- [`../ARCHITECTURE.md`](../ARCHITECTURE.md) — page maitre architecture avec schema global et navigation
- [`comprendre-le-projet.md`](comprendre-le-projet.md) — lecture rapide pour comprendre le projet et en parler
- [`comment-ca-marche.md`](comment-ca-marche.md) — explication technique progressive des sprints avec liens vers le code
- [`glossaire.md`](glossaire.md) — definitions courtes des composants et concepts
- [`architecture/README.md`](architecture/README.md) — sous-pages detaillees d'architecture
- [`architecture/08-agentic-mvp.md`](architecture/08-agentic-mvp.md) — cadrage du MVP agentique et separation produit/tests
- [`gitops-structure.md`](gitops-structure.md) — structure GitOps locale, conventions et validations
- [`ansible-structure.md`](ansible-structure.md)
- [`cours-accelere-ansible.md`](cours-accelere-ansible.md) — cours rapide Ansible avec exemples tires du projet
- [`concepts-sprint-0.md`](concepts-sprint-0.md)
- [`concepts-sprint-1.md`](concepts-sprint-1.md)
- [`decouverte-ministack.md`](decouverte-ministack.md)

## Schemas

- [`diagrams/README.md`](diagrams/README.md) — catalogue des schemas, sources
  editables, rendus et documents consommateurs.

## Repertoires annexes

- [`adr/`](adr) — decisions structurantes, dont [`ADR-001-gitlab-com-for-ci.md`](adr/ADR-001-gitlab-com-for-ci.md)
- [`evidence/`](evidence)
